...
# get protocol: 8 bits
def get_protocol(protocolNr):
    protocol_file = open('Protocol.txt', 'r')
...
def main():
    ...
    strihl = str(IHL*4)
    gettos = getTOS(TOS)
    totlen = str(totalLength)
    strid = str(ID)
    stroff = str(fragmentOffset)
    strttl = str(TTL)
    proto = get_protocol(protocolNr)
    strchksum = str(checksum)
    strpayload = data[20:]
    strsrc = str(sourceAddress)
    strdst = str(destinationAddress)

    print "An IP packet with the size %i was captured." % (unpackedData[2])
    print "Raw data: " + strdata
    print "\nParsed data"
    print "Version:\t\t" + strver #IP Version
    print "Header Length:\t\t" + strihl + " bytes" #
    print "Type of Service:\t" + gettos
    print "Length:\t\t\t" + totlen
    print "ID:\t\t\t" + str(hex(ID)) + " (" + strid + ")"
    print "Flags:\t\t\t" + getFlags(flags)
    print "Fragment offset:\t" + stroff
    print "TTL:\t\t\t" + strttl
    print "Protocol:\t\t" + proto
    print "Checksum:\t\t" + strchksum
    print "Source:\t\t\t" + strsrc
    print "Destination:\t\t" + strdst
    print "Payload:\n" + strpayload
    print "\n\n\n"
    # disabled promiscuous mode

# saving data to database
    db = MySQLdb.connect("localhost","root","","netsec1")
    cursor = db.cursor();
    sql = "INSERT INTO sniff (ver, ihl, length, ip_id, ttl, chksum, src, dst, rawdata,payload) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (strver, strihl, totlen, strid, strttl, strchksum, sourceAddress, destinationAddress,strdata,strpayload)
    try:
        cursor.execute(sql)
        db.commit()
    except:
